...
description: Build or refine frontend modals, dialogs, sheets, drawers, confirm dialogs, overlay forms, and interruptive action surfaces on top of an existing stack, theme, and product UI DNA. Use when Codex needs to implement a modal flow, confirmation dialog, destructive-action prompt, side sheet, slide-over panel, or focused overlay step while preserving the current component family and handling trigger context, focus, layering, dismissal, scroll locking, responsive behavior, and action clarity correctly. Prefer this skill after the frontend stack and visual direction already exist. If the stack is not chosen yet, use `frontend-foundation-builder` first. If the theme still needs to be derived from Figma or screenshots, use `frontend-figma-to-theme` first. If the task is really a tooltip, dropdown, popover, command palette, or a broader page, auth, checkout, or detail flow where the modal is only one part, use the matching skill first. If the work is only a tiny tweak on an existing modal, use the nearest skill first.
---
...
- the main task is specifically a tooltip, hover hint, or lightweight contextual explanation
- the main task is specifically a dropdown menu or anchored option list
- the main task is specifically a popover or contextual mini-panel rather than a blocking overlay
